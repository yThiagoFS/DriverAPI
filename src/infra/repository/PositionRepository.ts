...
export class PositionRepositoryDatabase implements PositionRepository {
    ...
    async savePosition(position: Position) : Promise<void> {
        await this.databaseConnection.query("insert into db.position (position_id, ride_id, lat, long, date) values($1, $2, $3, $4, $5)", [position.positionId, position.rideId, position.coord.getLat(), position.coord.getLong(), position.date]);
